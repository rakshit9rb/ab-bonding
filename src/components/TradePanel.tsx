...
import {
  CLOB_URL, CTF_EXCHANGE, NEG_RISK_CTF_EXCHANGE, USDC_ADDRESS,
  calcMarketPreview, calcSellPreview, signAndPlaceOrder,
  getUsdcBalance, getUsdcAllowance, approveUsdc,
  OrderBook, OrderPreview, OrderType,
} from '@/lib/polymarket'
import {
  getOrCreateCreds, buildL2Headers, clearCreds, ApiCredentials,
...
function OrderBookDisplay({ book, outcome }: { book: OrderBook; outcome: Outcome }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="grid grid-cols-3 text-[10px] font-semibold uppercase tracking-wider pb-1 mb-1"
        ...
        {asks.map((ask, i) => {
          ...
          return (
            <div key={i} className="relative grid grid-cols-3 text-[11px] font-mono py-[2px] px-1 rounded-sm overflow-hidden">
              <div className="absolute inset-0" style={{ background: 'rgba(220,38,38,0.12)', width: `${pct}%`, left: 'auto', right: 0 }} />
...
function DepositPanel({ address, usdcBalance }: { address: string; usdcBalance: number | null }) {
  ...
  return (
    <div className="rounded-xl p-4 mb-4" style={{ background: '#161b22', border: '1px solid #2d3748' }}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: '#6b7280' }}>Deposit USDC to Polygon</span>
        {usdcBalance !== null && (
          <span className="text-[12px] font-mono font-semibold" style={{ color: usdcBalance > 0 ? '#4ade80' : '#6b7280' }}>
      ...
      <p className="text-[12px] mb-3" style={{ color: '#9ca3af' }}>
        Send <strong style={{ color: '#e5e7eb' }}>USDC</strong> on <strong style={{ color: '#e5e7eb' }}>Polygon</strong> to your wallet address below.
      </p>

      <div className="flex gap-2 mb-3">
        <code className="flex-1 px-2 py-1.5 rounded-lg text-[11px] font-mono truncate" style={{ background: '#0d1117', border: '1px solid #374151', color: '#9ca3af' }}>
          {address}
        </code>
        <button onClick={copy}
          className="px-3 py-1.5 rounded-lg text-[12px] font-semibold cursor-pointer transition-all shrink-0"
          style={{ background: copied ? 'rgba(5,150,80,0.2)' : 'rgba(255,255,255,0.06)', border: '1px solid #374151', color: copied ? '#4ade80' : '#9ca3af' }}>
          {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
...
export default function TradePanel({ bond, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    if (orderType === 'GTC') {
      ...
      setPreview({ avgPrice: price, shares, totalCost: tradeDir === 'BUY' ? num : shares * price, potentialReturn: tradeDir === 'BUY' ? shares - num : shares * price, priceImpact: 0 })
  ...
  const ensureCreds = useCallback(async (walletClient: any): Promise<ApiCredentials | null> => {
    ...
    setStatus('authing'); setStatusMsg('Sign once to enable trading…')
    const c = await getOrCreateCreds(walletClient, wallet!.address)
    if (c) { setCreds(c); setStatus('idle'); setStatusMsg('') }
    else   { setStatus('error'); setStatusMsg('Auth failed — please try again') }
    return c
  }, [creds, wallet])
  ...
  const isLoading         = status === 'loading' || status === 'approving' || status === 'switching' || status === 'authing'

  return (
    <div className="col-span-full overflow-hidden" style={{ borderBottom: '1px solid var(--border)', background: '#0d1117' }}>
        ...
        <div className="p-4 border-r" style={{ borderColor: '#1f2937' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-3 text-[12px] font-mono">
              ...
              <span style={{ color: '#6b7280' }}>{lastPrice ? `Last ${(lastPrice * 100).toFixed(1)}¢` : `Mid ${(midPrice * 100).toFixed(1)}¢`}</span>
            </div>
          </div>
          {book
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          {wallet?.address && (
            <div className="flex items-center justify-between text-[12px]">
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full inline-block" style={{ background: onPolygon ? '#4ade80' : '#f87171' }} />
                <span style={{ color: '#6b7280' }}>{onPolygon ? 'Polygon' : 'Wrong network'}</span>
              </div>
              {usdcBalance !== null && (
                <span style={{ color: '#6b7280' }}>
                  Balance: <span className="font-mono" style={{ color: '#9ca3af' }}>${usdcBalance.toFixed(2)}</span>
                </span>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {(['BUY', 'SELL'] as TradeDir[]).map(d => (
              <button key={d} onClick={() => { setTradeDir(d); setAmount('') }}
                className="py-1.5 rounded-md text-[12px] font-semibold cursor-pointer transition-all"
                style={{
                  background: tradeDir === d ? (d === 'BUY' ? 'rgba(5,150,80,0.2)' : 'rgba(220,38,38,0.15)') : 'rgba(255,255,255,0.04)',
          ...
          <div>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[13px] font-mono" style={{ color: '#4b5563' }}>
                {tradeDir === 'BUY' ? '$' : '#'}
              </span>
          ...
          {preview && (
            <div className="rounded-lg p-2.5 text-[12px] font-mono" style={{ background: '#161b22', border: '1px solid #1f2937' }}>
              ...
              <div className="flex justify-between mb-1">
                <span style={{ color: '#6b7280' }}>{tradeDir === 'BUY' ? 'Shares' : 'Shares sold'}</span>
                <span style={{ color: '#e5e7eb' }}>{preview.shares.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: '#6b7280' }}>{tradeDir === 'BUY' ? 'Max profit' : 'USDC received'}</span>
                <span style={{ color: tradeDir === 'BUY' ? '#4ade80' : '#60a5fa' }}>
                  {tradeDir === 'BUY' ? `+$${preview.potentialReturn.toFixed(2)}` : `$${preview.totalCost.toFixed(2)}`}
                </span>
              </div>
          ...
          {statusMsg && (
            <div className="text-[12px] px-2.5 py-1.5 rounded-lg" style={{
              background: status === 'success' ? 'rgba(5,150,80,0.1)' : status === 'authing' ? 'rgba(59,130,246,0.1)' : 'rgba(220,38,38,0.1)',
              color:      status === 'success' ? '#4ade80' : status === 'authing' ? '#60a5fa' : '#f87171',
            }}>
              {statusMsg}
            </div>
          )}
          ...
          {!authenticated ? (
            ...
            <button onClick={handleTrade}
              ...
              style={{
                background: tradeDir === 'BUY'
                  ? (outcome === 'YES' ? '#059650' : '#dc2626')
                  : 'rgba(59,130,246,0.8)',
                color: '#fff', border: 'none',
              }}>
              {isLoading
                ? (status === 'authing' ? 'Authenticating…' : status === 'switching' ? 'Switching…' : 'Placing…')
