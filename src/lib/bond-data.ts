...
function toMarketList(value: unknown): RawMarket[] {
  if (Array.isArray(value)) return value.filter((item): item is RawMarket => typeof item === 'object' && item !== null)
  if (isMarketEnvelope(value) && Array.isArray(value.markets)) {
    return value.markets.filter((item): item is RawMarket => typeof item === 'object' && item !== null)
  }
  return []
}
...
function getMarketSlug(market: RawMarket): string {
  return String(
    (Array.isArray(market.events) && market.events.length > 0
      ...
      : null) || market.slug || market.conditionId || ''
  )
}
...
const CATEGORY_RULES: [string, RegExp][] = [
  ['Crypto', /\b(bitcoin|btc|ethereum|eth|crypto|solana|sol|xrp|ripple|doge|dogecoin|bnb|usdc|usdt|stablecoin|defi|nft|blockchain|altcoin|coinbase|binance|token|web3|memecoin|pepe|shib)\b/i],
  ...
  ['Finance', /\b(fed|federal reserve|inflation|gdp|interest rate|recession|cpi|unemployment|jobs report|rate cut|rate hike|treasury|dollar|euro|forex|oil|crude|brent|gold|silver|commodity|imf|world bank|tariff|trade war|deficit|debt ceiling|stock|s&p|nasdaq|dow|earnings|ipo|acquisition|merger|hedge fund|bonds|yield|equity|etf|fomc|ecb|bank of england|boe)\b/i],
  ['Sports', /\b(nba|nfl|mlb|nhl|ufc|fifa|premier league|champions league|super bowl|world cup|olympics|championship|nascar|formula 1|f1|tennis|wimbledon|grand slam|playoffs|finals|mvp|transfer|draft|cricket|ipl|rugby|golf|pga)\b/i],
  ['Technology', /\b(ai|artificial intelligence|openai|chatgpt|gpt|llm|apple|google|microsoft|meta|amazon|nvidia|tesla|spacex|robot|autonomous|chip|semiconductor|iphone|android|antitrust|ipo|startup|doge|department of government efficiency)\b/i],
  ['Science', /\b(climate|global warming|co2|emissions|pandemic|vaccine|fda|drug approval|cancer|virus|covid|health|nasa|space|moon|mars|asteroid|discovery|earthquake|volcano)\b/i],
  ['Weather', /\b(hurricane|typhoon|cyclone|tornado|flood|drought|storm|blizzard|wildfire|heat wave|el ni[nñ]o|la ni[nñ]a|monsoon|snowfall|rainfall|temperature record|sea level)\b/i],
  ['Entertainment', /\b(oscar|grammy|emmy|golden globe|box office|movie|film|music|album|artist|celebrity|award|netflix|disney|hollywood|tv show|streaming)\b/i],
]
...
function getCategory(market: RawMarket): string {
  ...
  if (apiCategory && apiCategory.toLowerCase() !== 'other' && apiCategory.toLowerCase() !== 'unknown') {
    return apiCategory
  }
...
async function fetchMarketUniverse(sortKeys: readonly string[]): Promise<RawMarket[]> {
  const pageResults = await Promise.allSettled(buildMarketUrls(sortKeys).map((url) => fetchJson(url)))
  const markets = dedupeMarkets(
    pageResults.flatMap((result) => (result.status === 'fulfilled' ? toMarketList(result.value) : []))
  )

  if (markets.length > 0) return markets

  try {
    const fallback = await fetchJson(`${GAMMA_API_BASE}/markets?closed=false&limit=${ACTIVE_MARKET_PAGE_SIZE}`)
...
async function enrichLiquidityFromClob(bonds: Bond[], tokenIdToIndex: Map<string, number>) {
  ...
  for (let index = 0; index < tokenIds.length; index++) {
    ...
    if (result.status !== 'fulfilled' || typeof result.value !== 'object' || result.value === null) continue
