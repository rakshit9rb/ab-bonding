...
export function calcMarketPreview(
  ...
  for (const lvl of levels) {
    ...
    if (cost <= remaining) {
      totalShares += size;   totalSpent += cost;     remaining -= cost
    } else {
      const partial = remaining / price
      totalShares  += partial; totalSpent += remaining; remaining = 0
    }
  }
...
export async function signAndPlaceOrder({
  ...
  try {
    ...
    if (!res.ok) return { success: false, error: data.error ?? data.message ?? 'Order failed' }
